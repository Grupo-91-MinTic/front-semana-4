                        ...
                        <v-container grid-list-md>
                            <v-layout wrap>
                              <v-flex xs12 sm12 md12>
                                ...
                                <v-flex xs12 sm12 md12>
                                    <v-select v-model="selectCategoria" :items="nombresCategorias" label="Categoría"></v-select>
                                </v-flex>
                                <v-flex xs12 sm12 md12>
                                ...
                                <v-flex xs12 sm12 md12>
                                    <v-textarea v-model="caracteristicas" color="teal" counter maxlength="255" label="Características" hint="Agregar las características del producto">
                                    </v-textarea>
                                </v-flex>
                                <v-flex xs12 sm12 md12>
                                    <v-text-field v-model="urlImage" label="url Imagen"></v-text-field>
                                </v-flex>
                ...
                <v-dialog v-model="adModal" max-width="290">
                    <v-card>
                        ...
                        <v-card-actions>
                            ...
                            <v-btn v-if="adAccion==1" text @click="activar()" color="orange darken-4" >
                                Activar
                            </v-btn>
                            <v-btn v-if="adAccion==2" text @click="desactivar()" color="orange darken-4" >
                                Desactivar
                            </v-btn>
                        </v-card-actions>
                    </v-card>
        ...
        methods: {
            ...
            validar(){
                ...
                if(this.codigo.length<1 || this.codigo.length>50){
                    this.validaMensaje.push('El codigo del producto debe tener entre 1-50 caracteres.');
                }
                ...
                if(this.descripcion.length<1){
                    this.validaMensaje.push('La descripción del producto no debe ser nula.');
                }
                if(this.caracteristicas.length<1){
                    this.validaMensaje.push('La característica del producto no debe ser nula.');
                }
            ...
            guardar(){
                ...
                if (this.editedIndex >-1){
                    ...
                    axios.post('articulo/update',{'codigo': this.codigo,'nombre':this.nombre,'descripcion':this.descripcion, 'caracteristicas' : this.caracteristicas, 'urlImage' : this.urlImage, 'categoriaId': idCat },configuracion)
                    ...
                    //Código para guardar
                    let idCat = me.idCategorias[me.nombresCategorias.indexOf(me.selectCategoria)];
                    axios.post('articulo/add',{'codigo': this.codigo,'nombre':this.nombre,'descripcion':this.descripcion, 'caracteristicas' : this.caracteristicas, 'urlImage' : this.urlImage, 'categoriaId': idCat },configuracion)
                    .then(function(response){
                        me.limpiar();
                        me.close();
                        me.listar();
